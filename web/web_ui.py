...
from src.router.executors import crawl_keywords_playwright, crawl_keywords_pywire
...
async def serve_index() -> FileResponse:
    ...
    if not index_path.exists():
        raise HTTPException(status_code=500, detail="前端文件缺失，请检查 webui 目录")
    return FileResponse(index_path)


@app.post("/api/crawl")
async def api_crawl(payload: Dict[str, Any]) -> JSONResponse:
    """兼容旧入口：直接走 Playwright（E）。"""
